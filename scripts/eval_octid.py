"""Cross-cohort zero-shot evaluation: Kermany-trained OCT-SSRet on the OCTID
test cohort. Mapping from OCTID disease classes to Kermany 4-class is
defined in src/data.py.OCTIDDataset.OCTID_TO_KERMANY and is honestly
...
def main():
    ...
    loader = get_octid_loader(batch_size=args.batch, image_size=sa["image_size"], num_workers=2)
    ...
    try:
        # OCTID may not include all 4 classes; one-vs-rest AUC handles this.
    ...
    out_path = Path(args.ckpt).with_name("eval_octid.json")
    with out_path.open("w") as f:
        json.dump(res, f, indent=2)

    print(f"\nOCTID zero-shot ({len(y)} images)")
